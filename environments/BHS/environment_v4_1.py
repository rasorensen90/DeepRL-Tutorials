    ...
    def getObs(self):
        ...
        for e in self.elems:
            if e.tote == None:
                ...
                obs.append([(self.src+self.dst).index(e.tote.dst)+1])
...
#                    obs.append([self.dst.index(e.tote.dst)+1])
    ...
    def setSpaces(self):
        ...
        #     obs_size.append(1)
        self.observation_space = spaces.Box(low=0, high=len([0]+self.src+self.dst), shape=(obs_size, self.numObsFeatures),dtype=np.int32)
...
#        print(self.obs.shape)
    ...
    def step(self,action=[], shortestPath=False, dynamic=False, dla=False, maxCongestion=1.0): 
        ...
        if (self.stepnumber >= self.steplimit):
            self.done = True
        return obs, reward, self.done, tote_info, action_
